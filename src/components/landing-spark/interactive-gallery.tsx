...
const processTemario = (temario?: string[]): GroupedTemarioItem[] => {
  ...
  for (const point of temario) {
    ...
    } else if (point.startsWith("\t•\t") && !currentSection) {
       const lastMain = grouped.find(g => g.type === 'main' && temario.includes(g.content || ''));
       if (lastMain) {
  ...
  // Handle cases like Automaquillaje where bullet points directly follow a main heading
  if (grouped.length > 0 && grouped[grouped.length-1].type === 'main') {
    const mainTitleItem = grouped[grouped.length-1];
    if (mainTitleItem && mainTitleItem.content) {
      ...
      if (mainTitleIndex !== -1) {
          const itemsUnderMain = temario.slice(mainTitleIndex + 1)
                                  .filter(p => p.startsWith("\t•\t") && !isNumberedHeading(p))
                                  .map(p => p.replace(/^\t•\t/, '').trim());
          if(itemsUnderMain.length > 0 && !grouped.find(g => g.type === 'section' && g.items && JSON.stringify(g.items) === JSON.stringify(itemsUnderMain))) {
            ...
            {galleryItems.map((item) => (
              <React.Fragment key={item.id}>
                ...
                {isMobile && activeItem.id === item.id && (
                  <div className="mt-6 space-y-4 p-4 bg-card rounded-lg shadow-lg">
                          ...
                          <ScrollArea className="h-[200px] w-full pr-4">
                            <ul className="space-y-2">
                              {groupedTemarioState.map((group, groupIndex) => {
                                if (group.type === 'main') {
                                  return <li key={`main-mob-${groupIndex}`} className="mb-3"><h4 className="text-base font-medium text-accent mt-2 mb-1">{group.content}</h4></li>;
                                }
                                if (group.type === 'section') {
                                  const sectionsOnly = groupedTemarioState.filter(g => g.type === 'section');
                                  const isEffectivelyLastSection = sectionsOnly.length > 0 && sectionsOnly[sectionsOnly.length - 1].title === group.title && JSON.stringify(sectionsOnly[sectionsOnly.length - 1].items) === JSON.stringify(group.items);
                                  return (
                                    <li key={`section-mob-${groupIndex}`} className="flex items-start">
                                      ...
                                      <div className="flex-1" style={{ '--section-content-height': `${(group.items?.length || 0) * 1.5}rem` } as React.CSSProperties}>
                                        {group.title && <h5 className="text-sm font-semibold text-foreground mb-1.5">{group.title}</h5>}
                                        {group.items && group.items.length > 0 && (
                                          <ul className="space-y-1 pl-1">
                                            {group.items.map((subItem, itemIndex) => <li key={`item-mob-${itemIndex}`} className="text-sm text-foreground/80">{subItem}</li>)}
                                          </ul>
                                        )}
                                      </div>
                                    </li>
                                ...
                                if (group.type === 'raw') {
                                  return <li key={`raw-mob-${groupIndex}`} className="text-sm text-foreground/80 ml-7">{group.content}</li>;
                                }
                                return null;
                              })}
                            </ul>
                  ...
                  <CardContent>
                    <ScrollArea className="h-[250px] w-full pr-4">
                       <ul className="space-y-2">
                        {groupedTemarioState.map((group, groupIndex) => {
                          if (group.type === 'main') {
                            return ( <li key={`main-desk-${groupIndex}`} className="mb-3"> <h4 className="text-lg font-semibold text-accent mt-2 mb-1">{group.content}</h4> </li> );
                          }
                          if (group.type === 'section') {
                            const sectionsOnly = groupedTemarioState.filter(g => g.type === 'section');
                            const isEffectivelyLastSection = sectionsOnly.length > 0 && sectionsOnly[sectionsOnly.length - 1].title === group.title && JSON.stringify(sectionsOnly[sectionsOnly.length - 1].items) === JSON.stringify(group.items);
                            return (
                              <li key={`section-desk-${groupIndex}`} className="flex items-start">
                                <div className="flex flex-col items-center mr-3 shrink-0 pt-1">
                                  <div className="h-2.5 w-2.5 bg-accent rounded-full mt-1"></div>
                                  {!isEffectivelyLastSection && <div className="w-0.5 bg-accent flex-grow min-h-[calc(1rem_+_var(--section-content-height,1rem))]"></div>}
                                </div>
                                <div className="flex-1" style={{ '--section-content-height': `${(group.items?.length || 0) * 1.5}rem` } as React.CSSProperties}>
                                  ...
                                  {group.items && group.items.length > 0 && (
                                    <ul className="space-y-1 pl-1">
                                      {group.items.map((subItem, itemIndex) => ( <li key={`item-desk-${itemIndex}`} className="text-sm text-foreground/80">{subItem}</li> ))}
                                    </ul>
                                  )}
                                </div>
                              </li>
                           ...
                           if (group.type === 'raw') {
                             return <li key={`raw-desk-${groupIndex}`} className="text-sm text-foreground/80 ml-7">{group.content}</li>;
                           }
                          return null;
                        })}
                      </ul>
